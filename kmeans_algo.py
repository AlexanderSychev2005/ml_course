...
ro = lambda x_vect, m_vect: np.mean((x_vect - m_vect) ** 2) # метрика (среднеквадратическое отклонение), евклидова метрика
...
while n < 200:
    X = [[] for i in range(K)] # списки для точек каждого кластера

    for x_vect in x:
        r = [ro(np.array(x_vect), m_vect) for m_vect in ma] # расстояние от точки до каждого кластера
        k = np.argmin(r) # номер ближайшего кластера
        X[k].append(x_vect) # отнесение точки к кластеру
    ma = [np.mean(cluster, axis=0) for cluster in X] # пересчет центров кластеров

    plt.clf()
    # отображение кластеров
    for i in range(K):
        cluster = np.array(X[i]).T
        plt.scatter(cluster[0], cluster[1], color=COLORS[i], s=10)
